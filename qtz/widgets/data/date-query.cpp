...
DateQuery::DateQuery(QWidget* parent) :
    ...
    connect(ui->dateEditEq, SIGNAL(dateChanged(QDate)), this,
            SLOT(makeStatement()));
    connect(ui->dateEditGt, SIGNAL(dateChanged(QDate)), this,
            SLOT(makeStatement()));
    connect(ui->dateEditLe, SIGNAL(dateChanged(QDate)), this,
            SLOT(makeStatement()));
    connect(ui->dateEditB1, SIGNAL(dateChanged(QDate)), this,
            SLOT(makeStatement()));
    connect(ui->dateEditB2, SIGNAL(dateChanged(QDate)), this,
            SLOT(makeStatement()));
    ...
    switch(ui->stackedWidget->currentIndex()) {
        case 0: // equal
            m_statement = QString("%1 = '%2'").arg(fieldName).arg(
                              ui->dateEditEq->date().toString(Qt::ISODate));
            break;
        case 1: // greater
            m_statement = QString("%1 > '%2'").arg(fieldName).arg(
                              ui->dateEditGt->date().toString(Qt::ISODate));
            break;
        case 2: // less
            m_statement = QString("%1 < '%2'").arg(fieldName).arg(
                              ui->dateEditLe->date().toString(Qt::ISODate));
            break;
        case 3: // between
