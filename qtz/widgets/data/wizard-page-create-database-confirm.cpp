...
WizardPageCreateDatabaseConfirm::WizardPageCreateDatabaseConfirm(QWidget *parent) :
    QWizardPage(parent),
    ui(new Ui::WizardPageCreateDatabaseConfirm)
{
    ui->setupUi(this);
    connect(ui->toolButtonShowHide,SIGNAL(toggled(bool)),this,SLOT(showHidePasswrod(bool)));
}
...
{
    Database::Type providerType = static_cast<Database::Type>(wizard()->property("providerCode").toUInt());
    QString providerText = DataProviderInformation::getInstance()->getProviderInfo(providerType).providerName();
