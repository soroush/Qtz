    ...
    ui(new Ui::textEditorWindow)
{
    ...
    connect(ui->editor,SIGNAL(undoAvailable(bool)),ui->actionUndo,SLOT(setEnabled(bool)));

    connect(ui->actionRedo,SIGNAL(triggered()),ui->editor,SLOT(redo()));
    connect(ui->editor,SIGNAL(redoAvailable(bool)),ui->actionRedo,SLOT(setEnabled(bool)));

    connect(ui->actionCut,SIGNAL(triggered()),ui->editor,SLOT(cut()));
    connect(ui->editor,SIGNAL(copyAvailable(bool)),ui->actionCopy,SLOT(setEnabled(bool)));

    connect(ui->actionCopy,SIGNAL(triggered()),ui->editor,SLOT(copy()));
    connect(ui->editor,SIGNAL(copyAvailable(bool)),ui->actionCut,SLOT(setEnabled(bool)));
    ...
    connect(ui->actionUnderline,SIGNAL(triggered(bool)),this,SLOT(underline(bool)));
    connect(ui->actionStrickout,SIGNAL(triggered(bool)),this,SLOT(strickout(bool)));
    connect(fontCombo,SIGNAL(currentFontChanged(QFont)),this,SLOT(changeFont(QFont)));

    connect(ui->editor, SIGNAL(cursorPositionChanged()),this,SLOT(updateStatus()));
    connect(ui->editor, SIGNAL(undoAvailable(bool)),ui->actionUndo,SLOT(setEnabled(bool)));
    connect(ui->editor, SIGNAL(redoAvailable(bool)),ui->actionRedo,SLOT(setEnabled(bool)));
}
