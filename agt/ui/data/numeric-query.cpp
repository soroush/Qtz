...
{
    ui->setupUi(this);
    connect(ui->doubleSpinBoxEq,SIGNAL(valueChanged(double)),this,SLOT(makeStatement()));
    connect(ui->doubleSpinBoxGt,SIGNAL(valueChanged(double)),this,SLOT(makeStatement()));
    connect(ui->doubleSpinBoxLe,SIGNAL(valueChanged(double)),this,SLOT(makeStatement()));
    connect(ui->doubleSpinBoxB1,SIGNAL(valueChanged(double)),this,SLOT(makeStatement()));
    connect(ui->doubleSpinBoxB2,SIGNAL(valueChanged(double)),this,SLOT(makeStatement()));
...
void NumericQuery::makeStatement()
{
    ...
    {
    case 0: // equal
        m_statement = QString("%1 = %2").arg(fieldName).arg(ui->doubleSpinBoxEq->value());
        break;
    case 1: // greater
        m_statement = QString("%1 > %2").arg(fieldName).arg(ui->doubleSpinBoxGt->value());
        break;
    case 2: // less
        m_statement = QString("%1 < %2").arg(fieldName).arg(ui->doubleSpinBoxLe->value());
        break;
    case 3: // between
        m_statement = QString("%1 BETWEEN %2 AND %3").arg(fieldName).arg(ui->doubleSpinBoxB1->value())
